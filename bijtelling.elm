        ...
        berekening schaal =
            let
                verschil =
                    bedrag - schaal.start
            in
                if bedrag > schaal.start then
                    if verschil < schaal.eind || schaal.start == schaal.eind then
                        verschil * schaal.percentage / 100
                    else
                        (schaal.eind - schaal.start) * schaal.percentage / 100
                else
                    0
    in
...
berekenBijtelling model =
    let
        belasting =
            verschuldigd model.jaarSalaris

        bijtelling =
            verschuldigd (model.jaarSalaris + (model.catalogusWaarde * model.bijtelling / 100))

        maandelijks =
            (bijtelling - belasting) / 12
    in
